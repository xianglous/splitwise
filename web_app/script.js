...
async function getTotalOwed(user) {
  return BlockchainSplitwise.getTotalOwed(user);
}
...
async function getLastActive(user) {
  ...
  for (var i = 0; i <function_calls.length; i++) {
    if (function_calls[i].from == user.toLowerCase() ||
      function_calls[i].args[0].toLowerCase() == user.toLowerCase()) {
...
async function getNeighbors(node) {
  return BlockchainSplitwise.getCreditors(node);
}
...
async function doBFS(start, end, getNeighbors) {
  ...
  while (queue.length > 0) {
    ...
    if (lastNode.toLowerCase() === end.toString().toLowerCase()) {
      ...
      // console.log(neighbors, amounts);
      neighbors.forEach((neighbor) => {
        if (cur.includes(neighbor)) return;
        queue.push(cur.concat([neighbor]));
      });
    }
  }
  return [];
